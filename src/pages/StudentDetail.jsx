    ...
    <Section title="Classe">
      <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
        <div>
          ...
          <input className={inp} placeholder="6ème A, CE2…" value={form.class}
...
function SectionPresences({ studentId }) {
  ...
  return (
    <Section title="Présences"
      ...
      {showForm && (
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 mb-3 space-y-2">
          ...
          <div className="flex gap-2">
            <button onClick={add} disabled={saving} className="flex items-center gap-1 bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs hover:bg-blue-700 disabled:opacity-50">
              <Check size={12} /> {saving ? '…' : 'OK'}
            </button>
            <button onClick={() => setShowForm(false)} className="flex items-center gap-1 border border-gray-200 px-3 py-1.5 rounded-lg text-xs text-gray-500">
              <X size={12} /> Annuler
            </button>
          </div>
        </div>
      ...
      {records.length === 0 ? (
        <p className="text-gray-400 text-sm text-center py-4 bg-white rounded-xl border border-gray-200">Aucune absence enregistrée.</p>
      ) : (
            ...
            return (
              <div key={r.id} className="bg-white border border-gray-200 rounded-xl px-4 py-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  {r.reason && <span className="text-xs text-gray-400 hidden sm:inline">{r.reason}</span>}
                </div>
...
function SectionNotes({ studentId }) {
  ...
  const avg = grades.length
    ? (grades.reduce((s, g) => s + g.value * g.coefficient, 0) /
       grades.reduce((s, g) => s + g.coefficient, 0)).toFixed(1)
    : null
  ...
  return (
    <Section title="Notes"
        ...
        {TERMS.map(t => (
          <button key={t} onClick={() => setTerm(t)}
            className={`px-3 py-1 rounded-lg text-xs font-medium transition-colors
              ${term === t ? 'bg-blue-600 text-white' : 'bg-white border border-gray-200 text-gray-500'}`}>
            {t}
          </button>
        ))}
      ...
      {showForm && (
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 mb-3 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <div><label className="text-xs text-gray-500 mb-1 block">Matière</label>
        ...
        <div className="space-y-2">
          {grades.map(g => (
            <div key={g.id} className="bg-white border border-gray-200 rounded-xl px-4 py-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <span className={`font-bold text-sm ${g.value >= 10 ? 'text-green-600' : 'text-red-500'}`}>
                  {g.value}/20
                </span>
...
function SectionAppreciations({ studentId }) {
  ...
  return (
    <Section title="Appréciations"
      ...
      {showForm && (
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 mb-3 space-y-2">
          <div><label className="text-xs text-gray-500 mb-1 block">Appréciation</label>
            <textarea rows={3} className={inp} placeholder="Bon trimestre…" value={form.text}
              onChange={e => setForm(f => ({ ...f, text: e.target.value }))} /></div>
      ...
      {comments.length === 0 ? (
        <p className="text-gray-400 text-sm text-center py-4 bg-white rounded-xl border border-gray-200">Aucune appréciation.</p>
      ) : (
        <div className="space-y-2">
          {comments.map(c => (
      ...
      {showForm && (
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 mb-3 space-y-2">
          <p className="text-xs font-medium text-gray-600">
            {pendingAmount > 0 ? `+ ${pendingAmount} bon point` : `${pendingAmount} bon point`}
          </p>
          <input className={inp} placeholder="Motif (optionnel)" value={reason}
            onChange={e => setReason(e.target.value)} />
          <div className="flex gap-2">
            <button onClick={add} disabled={saving}
              className="flex items-center gap-1 bg-amber-500 text-white px-3 py-1.5 rounded-lg text-xs hover:bg-amber-600 disabled:opacity-50">
              <Check size={12} /> {saving ? '…' : 'Valider'}
            </button>
            <button onClick={() => setShowForm(false)}
              className="flex items-center gap-1 border border-gray-200 px-3 py-1.5 rounded-lg text-xs text-gray-500">
              <X size={12} /> Annuler
            </button>
          </div>
        </div>
        ...
        {entries.slice(0, 10).map(e => (
          <div key={e.id} className="bg-white border border-gray-200 rounded-xl px-4 py-2.5 flex items-center justify-between group">
            <div className="flex items-center gap-2">
              <span className={`font-bold text-sm ${e.amount > 0 ? 'text-amber-500' : 'text-red-500'}`}>
                {e.amount > 0 ? '+' : ''}{e.amount}
              </span>
...
export default function StudentDetail() {
  ...
  if (!student) return <div className="p-6 text-gray-400 text-sm">Élève introuvable.</div>
